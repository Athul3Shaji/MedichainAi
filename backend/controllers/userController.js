...
const update_user = async (req, res) => {
  try {
    ...
    // Find the user first to get existing profileImage
    const user = await User.findById(id);
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }
    let updateData = { username, email, profileImage: user.profileImage };
...
module.exports = { registerUser, loginUser, get_user, update_user, get_users };